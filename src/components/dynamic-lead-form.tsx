...
export interface DynamicLeadFormValues {
  firstName: string;
  middleName?: string;
  lastName: string;
  email: string;
...
const fieldClass =
  "w-full px-3.5 py-2.5 rounded-lg border border-slate-200 text-sm bg-white focus:outline-none focus:ring-2 focus:ring-[#1098F7] focus:border-transparent";
...
function DynamicField({ field, register, errors }: { field: FieldDef; register: UseFormRegister<FormValues>; errors: FieldErrors<FormValues> }) {
  ...
  // Path into a dynamically-shaped object react-hook-form can't type statically —
  // scoped `as never` casts here are the deliberate escape hatch for that.
...
/** Renders an event's admin-defined customFields (from the field-builder wizard step)
 *  as a validated form, for any template other than Education Fair — which keeps its
 *  own hand-written form in collect/page.tsx instead of going through this. */
export function DynamicLeadForm(props: {
  fields: FieldDef[];
  onSubmit: (values: DynamicLeadFormValues) => Promise<void>;
  submitting: boolean;
  submitError: string;
}) {
  ...
  return (
    <form onSubmit={submit} className="space-y-5">
      <div className="bg-white rounded-xl border border-slate-200 p-5 space-y-4">
        <h2 className="text-xs font-semibold text-slate-500 uppercase tracking-wider">Contact Information</h2>
        <div className="grid grid-cols-2 gap-3">
          ...
          <div>
            ...
            <input {...register("middleName")} className={fieldClass} placeholder="Chukwudi" />
          </div>
        </div>
        <div>
