...
import { Users, CalendarCheck2, CheckCircle2, GraduationCap, Globe2, BookMarked, Clock, MinusCircle, CalendarDays } from "lucide-react";
...
export default function AnalyticsPage() {
  ...
  const byDest = destinations
    .map((d) => ({ dest: d, count: leads.filter((l) => l.destinationId === d.id).length }))
    .filter((x) => x.count > 0)
    .sort((a, b) => b.count - a.count);

  const byLevel = (["BSc", "MSc", "PhD"] as const).map((level) => ({
    level,
    count: leads.filter((l) => l.levelOfInterest === level).length,
  }));
                ...
                <div key={dest.id}>
                  <div className="flex items-center justify-between mb-1.5">
                    ...
                    <span className="text-sm font-semibold text-slate-900">
                      {count} <span className="text-xs font-normal text-slate-400">({pct(count, leads.length)}%)</span>
                    </span>
                  </div>
